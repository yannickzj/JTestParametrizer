package ca.uwaterloo.eclipse.refactoring.cloneinfowriter;
...
import ca.uwaterloo.eclipse.refactoring.Activator;
import ca.uwaterloo.eclipse.refactoring.diff.TextDiff;
import ca.uwaterloo.eclipse.refactoring.diff.TextDiff.Diff;
import ca.uwaterloo.eclipse.refactoring.diff.TextDiff.Operation;
...
public class CloneInfoHTMLWriter extends CloneInfoWriter {
	...
	private final static String TEMPLATE_NAME = "template.html";
	...
	public void writeCloneInfo(ClonePairInfo pairInfo) {
		...
		try {					
			BufferedWriter writer = new BufferedWriter(new FileWriter(new File(outputFileSaveFolder + pairInfo.getCloneGroupID() + "-" + pairInfo.getClonePairID() + ".html")));
	...
	private void recursiveGetMappedTableRows(StringBuilder builder, CloneStructureNode root, int numberOfTabs,
		...
		if (root.getMapping() != null) {
			...
			if (nodeMapping instanceof PDGNodeMapping) {
				...
				System.out.println("NodeG1 type: " + pdgNodeMapping.getNodeG1().getStatement().getType());

				ASTNode astStatement1 = pdgNodeMapping.getNodeG1().getASTStatement();
